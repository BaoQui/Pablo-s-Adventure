...
func _ready() -> void:
	...
	# Force starting state to idle so you don't start falling
	current_animation = ""
	play_animation("idle")
...
func update_animation(input_dir: float) -> void:
	# Prioritize non-interruptible animations
...
func start_dash(input_dir: float):
	if input_dir == 0:
...
func punch():
	...
	play_animation("punch", 1.5)

	# Disable hitbox shortly after punch starts
	var hitbox_timer = get_tree().create_timer(0.1)
	hitbox_timer.timeout.connect(func():
		punch_hitbox.monitoring = false
	)

	# Safety timer: make sure we can move again even if animation_finished doesn't fire
	var punch_reset_timer = get_tree().create_timer(
		anim.sprite_frames.get_frame_count("punch") * (1.0 / anim.sprite_frames.get_animation_speed("punch")) / anim.speed_scale
	)
	punch_reset_timer.timeout.connect(func():
		is_punching = false
	)

func _on_punch_hitbox_body_entered(body: Node2D):
